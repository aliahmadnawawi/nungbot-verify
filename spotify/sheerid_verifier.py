"""
Program Utama Verifikasi Spotify Student (SheerID)
Dilengkapi dengan Proxy Support & Bahasa Indonesia
"""
import re
import random
import logging
...
class SheerIDVerifier:
    """SheerID Student Verifier (Spotify) dengan Proxy"""
    ...
    def __init__(self, verification_id: str):
        ...
        if proxy_url and len(proxy_url) > 5:
            logger.info(f"🛡️ Mengaktifkan Mode Stealth (Proxy Spotify Aktif)...")
    ...
    def parse_verification_id(url: str) -> Optional[str]:
        """Mengambil ID Verifikasi dari URL"""
    ...
    def verify(
        ...
        """Jalankan Proses Verifikasi Mahasiswa (Spotify)"""
        try:
            ...
            logger.info(f"ID Verif: {self.verification_id}")

            # Langkah 1: Generate Gambar
            ...
            if current_step in ["sso", "collectStudentPersonalInfo"]:
                logger.info("Langkah 3/4: Melewati Login Kampus (SSO)...")
            ...
            if not step4_data.get("documents"):
                raise Exception("Gagal mendapatkan link upload")
            ...
            logger.error(f"❌ Error Verifikasi: {e}")
            return {"success": False, "message": str(e), "verification_id": self.verification_id}


def main():
    """Fungsi Utama (Test Manual)"""
    import sys

    print("=" * 60)
    print("SheerID Spotify Verifier")
    print("=" * 60)
    print()
    ...
    if len(sys.argv) > 1:
        ...
        url = input("Masukkan Link Verifikasi Spotify: ").strip()
    ...
    if not verification_id:
        print("❌ Error: ID Verifikasi tidak ditemukan")
        sys.exit(1)
