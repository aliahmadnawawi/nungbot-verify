"""
Modul Generator Bukti Guru PNG - Bolt.new / PSU
Menghasilkan Kartu ID dan Surat Kerja secara otomatis.
"""
import random
...
def generate_psu_id():
    """Generate ID PSU Acak (9 digit, diawali angka 9)"""
    return f"9{random.randint(10000000, 99999999)}"

def generate_psu_email(first_name, last_name):
    """
    Generate Email Resmi PSU
    Format: nama.depan.nama.belakang + angka @psu.edu
    """
...
def generate_teacher_card_html(first_name: str, last_name: str, psu_id: str) -> str:
    """Membuat HTML untuk Kartu Tanda Pengenal Guru (Faculty ID Card)."""
...
def generate_employment_letter_html(
    ...
    """Membuat HTML untuk Surat Keterangan Kerja (Employment Verification Letter)."""
    name = f"{first_name} {last_name}"
    now = datetime.now()
...
# --- LOGIKA RENDER (PENGOLAH GAMBAR) ---

def _html_to_png_batch(html_list: list[tuple[str, int, int]]) -> list[bytes]:
    """
    Render banyak gambar sekaligus secara Async (Concurrent).
    
    Args:
        html_list: List berisi [(html_content, width, height), ...]

    Returns:
        list[bytes]: Daftar data gambar PNG
    """
    import asyncio
    try:
        from playwright.async_api import async_playwright
    except ImportError:
        raise Exception("Playwright belum terinstall! Jalankan: pip install playwright && playwright install chromium")

    async def render_single(html_content: str, width: int, height: int):
        """Render satu gambar secara async"""
        async with async_playwright() as p:
            # Gunakan argumen browser yang kompatibel dengan VPS/Docker
            ...
            # Buat context baru untuk setiap render
            context = await browser.new_context(
                viewport={'width': width, 'height': height},
                device_scale_factor=2, # Biar gambar tajam (Retina quality)
            )
            ...
            try:
                ...
                # Jika tinggi tidak ditentukan, hitung otomatis
                if height is None:
    ...
    async def render_all():
        """Jalankan semua tugas render secara bersamaan"""
...
def generate_images(first_name: str, last_name: str, school_id: str = '2565'):
    """
    Fungsi Utama: Generate 2 Gambar (ID Card & Surat Kerja)
    
    Args:
        first_name: Nama Depan
        last_name: Nama Belakang
        school_id: ID Sekolah (Default PSU)

    Returns:
        list[dict]: List berisi file name dan data gambar
    """
    psu_id = generate_psu_id()
    
    # Jabatan dan Departemen Acak agar terlihat variatif
    titles = [
        "Associate Professor",
        "Assistant Professor",
        "Teaching Professor",
    ...
    # 2. Siapkan antrian render (Concurrent)
    # Format: (HTML Content, Width, Height)
    html_list = [
        (card_html, 700, 1100),   # Ukuran ID Card
        (letter_html, 1300, 1600), # Ukuran Surat (A4 like)
    ]
...
if __name__ == '__main__':
    # --- TEST MANUAL ---
    print("Sedang generate gambar test...")
    try:
        assets = generate_images("John", "Doe")
        for asset in assets:
            with open(asset["file_name"], "wb") as f:
                f.write(asset["data"])
            print(f"✅ Berhasil membuat {asset['file_name']} ({len(asset['data'])} bytes)")
